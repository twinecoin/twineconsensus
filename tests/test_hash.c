...
START_TEST(test_tw_th_unlock_test_functions) {
  ...
  ck_assert_msg(test_u512_equal(digest, zero), "Test harness enabled by default for tw_th_sha_256");
  digest = tw_th_sha_512(NULL, 0);
  ck_assert_msg(test_u512_equal(digest, zero), "Test harness enabled by default for tw_th_sha_512");

  // Verify re-locks after an unlock
  unlock_test_harness(1);
  unlock_test_harness(0);

  digest = tw_th_sha_256(NULL, 0);
  ck_assert_msg(test_u512_equal(digest, zero), "Test harness enabled after re-lock for tw_th_sha_256");
  digest = tw_th_sha_512(NULL, 0);
  ck_assert_msg(test_u512_equal(digest, zero), "Test harness enabled after re-lock for tw_th_sha_512");
...
Suite * uint_suite(void) {
  ...
  tcase_add_test(tc_core, test_tw_hash_256);
  tcase_add_test(tc_core, test_tw_hash_512);
